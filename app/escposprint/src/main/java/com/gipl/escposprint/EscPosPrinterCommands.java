...
import com.gipl.escposprint.barcode.Barcode;
...
//    public static final byte[] TEXT_SIZE_BIG = new byte[]{0x1B, 0x21, 0x11};
    ...
    public EscPosPrinterCommands printText(String text, byte[] textSize, byte[] textColor, byte[] textReverseColor, byte[] textBold, byte[] textUnderline, byte[] textDoubleStrike) throws EscPosEncodingException {
        ...
        if (textSize == null) {
            textSize = EscPosPrinterCommands.TEXT_SIZE_SMALL;
        }
        if (textColor == null) {
...
//            }
     ...
     * @return Fluent interface
     */
    public EscPosPrinterCommands newLine(boolean removeExtraNewLine) throws EscPosConnectionException {
        return this.newLine(null, removeExtraNewLine);
    }
     ...
     * @return Fluent interface
     */
    public EscPosPrinterCommands newLine(byte[] align, boolean removeExtraNewLine) throws EscPosConnectionException {
        if (!this.printerConnection.isConnected()) {
            return this;
        }
